...
export function useMonsterData(tableName: string, debouncedSearchTerm: string, currentPage: number, levelFilter: string, raceFilter: string) {
  const { supabase } = useSupabase();
  const [records, setRecords] = useState<TPropMover[]>([]);
    ...
    try {
      ...
      if (debouncedSearchTerm) {
        ...
        const { data: nrows } = await supabase
          ...
          .select("szname")
          .or(
            `szname.ilike.%${term}%,dwlevel.ilike.%${term}%,dwrace.ilike.%${term}%`
          );
      ...
      if (levelFilter !== "all") {
        query = query.eq("dwlevel", levelFilter);
      }
      if (raceFilter !== "all") {
        query = query.eq("dwrace", raceFilter);
      }
      ...
      const monsters = (data as TPropMover[]) || [];
      setRecords(monsters);
      const total = count || 0;
      ...
      const keys = monsters.map((r) => r.szname).filter(Boolean) as string[];
      if (keys.length > 0) {
      ...
      toast.error("Unexpected error loading monsters");
    } finally {
      setLoading(false);
    }
  }, [supabase, tableName, debouncedSearchTerm, currentPage, levelFilter, raceFilter]);
